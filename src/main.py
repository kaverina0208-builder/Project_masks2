...
def main() -> None:
    ...
    status = input('Введите статус, по которому необходимо выполнить фильтрацию.'
          '\nДоступные для фильтровки статусы: EXECUTED, CANCELED, PENDING__').upper()
    ...
    while status not in status_list:
        ...
        status = input('Введите статус, по которому необходимо выполнить фильтрацию.'
              '\nДоступные для фильтровки статусы: EXECUTED, CANCELED, PENDING__').upper().strip()
    ...
    if len(result_5) == 0:
        ...
        if data_format == '1':
            for element in result_5:
                ...
                print(f'Сумма: {element["operationAmount"]["amount"]} {element["operationAmount"]["currency"]["name"]}')
        else:
            for element in result_5:
                print(f'{get_date(element["date"])}  {element.get("description", 0)}')
                if element.get("description", 0) == "Открытие вклада":
